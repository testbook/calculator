...
function calculateFun() {
    if (!inputNumberFirst.checkValidity() || !pattern.test(inputNumberFirst.value)) {
        ...
        let surfaceArea = Math.PI * radius * slantHeight;
        let totalArea = surfaceArea + Math.PI * Math.pow(radius,2);

        resultBoxFirst.value = slantHeight.toFixed(3);
        resultBoxSecond.value = volume.toFixed(3);
        resultBoxThird.value = surfaceArea.toFixed(3);
        resultBoxFourth.value = totalArea.toFixed(3);
    }
