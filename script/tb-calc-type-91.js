...
function calculateFun() {
    ...
    if(status === true) {
        ...
        let [a1, b1, c1, a2, b2, c2] = inputVars;

        let eqn1 = `${a1}x + ${b1}y = ${c1}`;
        let eqn2 = `${a2}x + ${b2}y = ${c2}`;

        console.log(eqn1, eqn2);
        let res = solveSimultaneousEquations(eqn1, eqn2);
...
function solveSimultaneousEquations(eqn1, eqn2) {
    // Extract the constant terms from the equations
    const c1 = eqn1.split('=')[1].trim();
    const c2 = eqn2.split('=')[1].trim();
  
    // Solve for one of the variables in terms of the other variable and the constant
    const var1Coeff = 1; // Coefficient of the variable being solved for in the first equation
    const var2Coeff = 1; // Coefficient of the other variable in the first equation
    const var1Const = c1 - var2Coeff; // Constant term in the first equation
    const var1Expr = `${var1Coeff}x + ${var1Const}`;
  
    // Substitute the expression for the solved variable into the second equation
    const eqn2WithVar1 = eqn2.replace(/y/g, `(${var1Expr})`);
  
    // Solve for the remaining variable
    const var2 = eval(eqn2WithVar1); // Assumes the remaining variable is y
  
    // Use the value of the solved variable to find the value of the other variable
    const var1 = (c1 - var2).toFixed(2); // Assumes the solved variable is x
  
    // Return the solutions as an object with properties x and y
