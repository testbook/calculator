...
window.onload = function () {
    switch (calcType) {
        case "simplify-fractions-calculator":
            break;
        default:
...
function calculateFun() {
    if (!inputNumberFirst.checkValidity() || !pattern.test(inputNumberFirst.value)) {
        ...
        let valFirst = Number(inputNumberFirst.value);
        let valSecond = Number(inputNumberSecond.value); 
        
        let frac = math.fraction(valFirst, valSecond);

        resultBoxFirst.value = frac.n
        resultBoxSecond.value = frac.d;
