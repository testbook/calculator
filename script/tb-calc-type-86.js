...
const inputAddons = [
    ...
    { prefix: "Height (h) ", suffix: "m" },
    { prefix: `Potential Energy ${'(gravitational)'.sub()} `, suffix: "J" },
];
...
window.onload = function () {
    switch (calcType) {
        case "gravitational-potential-energy-calculator":
            inputGenerator();
            resultBoxGenerator();
            break;
        default:
...
function calculateFun() {
    ...
    if(status === true) {
        ...
        let [m, h, gpe] = inputVars;
        let g = 9.8;
        

        if(m.toLowerCase() === 'x') {
            resultBoxFirst.value =  gpe / (g * h);
        }else if(h.toLowerCase() === 'x') {
            resultBoxFirst.value =  gpe / (m * g);
        }else if(gpe.toLowerCase() === 'x') {
            resultBoxFirst.value =  m * g * h;
        }else {
