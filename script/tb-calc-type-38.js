...
window.onload = function () {
    switch (calcType) {
        case "multiplicative-inverse-calculator":
            break;
        default:
...
function getGcd(divider, num1,num2){
    return divider/(math.gcd(num1,num2))
}
...
function calculateFun() {
    if (!inputNumberFirst.checkValidity() || !pattern.test(inputNumberFirst.value)) {
        ...
        let d1 = Number(inputNumberFirst.value);
        let d2 = Number(inputNumberSecond.value);
        resultBoxFirst.value = getGcd(d1, d1,d2)
        resultBoxSecond.value = getGcd(d2, d1,d2)
    }
