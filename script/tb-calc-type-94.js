...
const inputAddons = [
    { prefix: "Enter Value of X(separated by comma) ", suffix: "" },
    { prefix: "Enter Value of Y(separated by comma) ", suffix: "" },
];

const resultAddons = [
    { prefix: "No.of Inputs(N)", suffix: "" },
    { prefix: "Correlation Coefficient(r)", suffix: "" },
    { prefix: `Coefficient of Determination (r${"2".sup()})`, suffix: "" },
];
...
function calculateFun() {
    ...
    if(status === true) {
        ...
        let x = inputVars[0].split(',').map(Number);
        let y = inputVars[1].split(',').map(Number);
        if(x.length === y.length) {
            let n = x.length || y.length;
            let xx = x.map((item, index) => {return item*x[index];});
            let yy = y.map((item, index) => {return item*y[index];});
            let xy = x.map((item, index) => {return item*y[index];});
            let sumessX = summession(x);
            let sumessY = summession(y);
            let sumessXY = summession(xy);
            let sumessXX = summession(xx);
            let sumessYY = summession(yy);

            let rr = (n * sumessXY - sumessX * sumessY) / Math.sqrt( (n * sumessXX - Math.pow(sumessX, 2)) * (n * sumessYY - Math.pow(sumessY, 2)));
            
            resultBoxFirst.value = n;
            resultBoxSecond.value = Math.sqrt(Math.abs(rr));
            resultBoxThird.value = rr;
        }else {
            alert("Please provide same number of inputs in both fields");
        }
    }
}
