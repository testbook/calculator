...
const resultAddons = [
    { prefix: "a1 ^ <sup>b1</sup>", suffix: "" },
    { prefix: "a2 ^ <sup>b2</sup>", suffix: "" },
...
window.onload = function () {
    switch (calcType) {
        case "rational-exponents-calculator":
            inputGenerator();
            resultBoxGenerator();
            break;
        default:
...
function calculateFun() {
    ...
    if(status === true) {
        ...
        [a1, b1, a2, b2] = inputVars;
        resultBoxFirst.value = Math.pow(a1, b1);
        resultBoxSecond.value = Math.pow(a2, b2);
        resultBoxThird.value = Math.pow(a1, b1) * Math.pow(a2, b2);
    }
}
