...
const inputAddons = [
    { prefix: "Temperature (T) ", suffix: "J" },
    { prefix: "Rate Constant (K) ", suffix: "mol/sec" },
    { prefix: "Frequency Factor (A) ", suffix: `sec${"-1".sup()}` },
];

const resultAddons = [
    { prefix: `Activation Energy (E${"a".sub()})`, suffix: "J/mol" },
];
...
window.onload = function () {
    switch (calcType) {
        case "activation-energy-calculator":
            inputGenerator();
            resultBoxGenerator();
            break;
        default:
...
function calculateFun() {
    ...
    if(status === true) {
        ...
        let res = -1 * R * t * Math.log(k/a);

        resultBoxFirst.value = res.toFixed(3);
