...
const inputAddons = [
    { prefix: "Inertia (I) ", suffix: `Kg.m${'2'.sup()}` },
    { prefix: "Angular Frequency (ω) ", suffix: "rad/sec" },
    { prefix: "Angular Momentum (L) ", suffix: `Kgm${'2'.sup()}/s` },
];
...
window.onload = function () {
    switch (calcType) {
        case "angular-momentum-calculator":
            inputGenerator();
            resultBoxGenerator();
            break;
        default:
...
function calculateFun() {
    ...
    if(status === true) {
        ...
        if(i.toLowerCase() === 'x') {
            resultBoxFirst.value = l/w;
        }else if(w.toLowerCase() === 'x') {
            resultBoxFirst.value = l/i;
        }else if(l.toLowerCase() === 'x') {
            resultBoxFirst.value = i * w;
        }else {
