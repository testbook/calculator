...
const inputAddons = [
    ...
    { prefix: "Velocity at Impact (v) ", suffix: "m/s" },
    { prefix: "Time Contact during Impact (t)", suffix: "sec" },
    { prefix: "Impact Force (F) ", suffix: "N" },
];
...
function calculateFun() {
    ...
    if(status === true) {
        ...
        let [m, v, t, f] = inputVars;
        

        if(m.toLowerCase() === 'x') {
            resultBoxFirst.value = (f * t) / (2 * v);
        }else if(t.toLowerCase() === 'x') {
            resultBoxFirst.value = (2 * m * v) / f;
        }else if(v.toLowerCase() === 'x') {
            resultBoxFirst.value = (f * t) / (2 * m);
        }else if(f.toLowerCase() === 'x') {
            resultBoxFirst.value = (2 * m * v) / t;
        }else {
